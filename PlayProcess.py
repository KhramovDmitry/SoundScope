...
class PlayProcess:
    def run(self):
        ...
        pygame.display.set_icon(pygame.image.load(self.oGetFilePath.load_file('songs', 'temp_cover.jpg')))
        ...
        if self.width > self.height:
            self.bi_size = (self.width, self.width)
        else:
            self.bi_size = (self.height, self.height)

        self.album_image = load_image(self.oGetFilePath.load_file('songs', 'temp_cover.jpg'), c.smooth, (self.width - self.width//12, self.width - self.width//12)).convert_alpha()

        self.blurred_image = blur_image_with_pillow(self.oGetFilePath.load_file('songs', 'temp_cover.jpg')).convert_alpha()
        self.blurred_image = pygame.transform.smoothscale(self.blurred_image, self.bi_size)
        ...
        self.update_ui_elements(self.width, self.height)
                ...
                if event.type == pygame.QUIT:
                    ...
                    if self.play_button_rect.collidepoint(self.mouse_pos):
                        if self.button_state == 'play':
                            ...
                            self.play_button.button_image = load_image(c.play_button, c.smooth, (self.pb_width, self.pb_height))
                            self.button_state = 'pause'
                        elif self.button_state == 'pause':
                            pygame.mixer.music.unpause()
                            self.play_button.button_image = load_image(c.pause_button, c.smooth, (self.pb_width, self.pb_height))
                            self.button_state = 'play'
    ...
    def update_ui_elements(self, width, height):
        ...
        self.album_image_rect = self.album_image.get_rect(center=(self.width//2, self.height//3.6))
        self.song_frame_rect = self.song_frame.get_rect(center=(self.width//2, self.height//3.6))
        self.frame_glow_rect = self.frame_glow.get_rect(center=(self.width//2, self.height//3.6 + self.start_height//23))
        self.blurred_image_rect = self.blurred_image.get_rect(center=(self.width//2, self.height//2))

        self.play_button.center_x, self.play_button.center_y = self.pbx, self.pby
